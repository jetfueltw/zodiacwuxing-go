...
func TestGetWuxingNumber(t *testing.T) {
	...
	t.Run("2020 water", func(t *testing.T) {
		...
		assert.Equal([]int{8, 9, 16, 17, 24, 25, 38, 39, 46, 47}, wuxingNumber)
	})
	...
	t.Run("2020 earth", func(t *testing.T) {
		...
		assert.Equal([]int{1, 14, 15, 22, 23, 30, 31, 44, 45}, wuxingNumber)
	})
...
func TestGetNumberWuxing(t *testing.T) {
	...
	t.Run("2020 49", func(t *testing.T) {
		numberWuxing, err := GetNumberWuxing(2020, 49)
		if !assert.NoError(err) {
			return
		}

		assert.Equal("wood", numberWuxing)
	})
